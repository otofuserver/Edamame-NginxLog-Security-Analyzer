...
## バージョン情報
- **db_schema_spec version**: v1.0.2
- **最終更新**: 2026-03-11
- **変更概要**: block_ipクリーンアップ処理を専用バッチに分離し、設定値の参照方法を明記。
...
- カラム:
  ...
  - `service_type` ENUM('MONITOR_BLOCK','APP_LOGIN') NOT NULL — 監視対象ブロック/アプリ不正ログイン
  - `target_agent_name` VARCHAR(128) NULL — 監視対象ブロック時のエージェント名
  - `reason` VARCHAR(255) NOT NULL — 検知理由や手動メモ
  - `trigger_source` ENUM('MODSEC','WAF','RATE_LIMIT','MANUAL','OTHER') NOT NULL
