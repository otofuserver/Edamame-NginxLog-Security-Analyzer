...
- ブロックIP（APP_LOGIN用途）に該当するアクセスは 429 で汎用エラー表示を返し、フォームに再試行案��を含める。
...
- ブロック中IPと判定された場合は履歴だけ記録し、429でログイン画面を返す（メッセージ: 「ユーザー名またはパスワードが正しくありません。数分後に再試行してください。」）。
...
- Content-Security-Policy は nonce 付き script-src を付与し、style は self + unsafe-inline を許容。
- エラー/成功メッセージはログインフォーム内で表示し、汎用テキストで攻撃者に情報を与えない。
...
- `private boolean isLoginBlocked(...)`（AuthenticationService 側）判定結果に応じ 429 を返す

## セキュリティ/運用
- フォーム入力はサーバ側で再検証し、パスワードは `AuthenticationService` でハッシュ照合すること。
- 初期管理者パスワード（README 等に記載されている場合）は運用初回後に必ず変更する運用を徹底する。
- 429 返信時も HTML でフォームを返し続ける（JSON APIは使用しない）。

## 変更履歴
- 1.0.0 - 2025-12-30: 新規作成
- 1.1.0 - 2026-03-16: ブロックIP判定時の429応答と汎用メッセージ表示を追記
